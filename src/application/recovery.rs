...
mod integration_tests {
    ...
    fn test_exclusive_fullscreen_recovery() {
        ...
        while start.elapsed() < test_duration {
            match capture.capture_frame() {
                    ...
                    if recovery.record_timeout() {
                        ...
                        match capture.reinitialize() {
                            Ok(_) => {
                                ...
                                println!(
                                    "[REINIT] Success (backoff: {:?})",
                                    recovery.current_backoff()
                                );
                            }
                            Err(e) => {
                    ...
                    match capture.reinitialize() {
                        Ok(_) => {
                            ...
                            if device_not_available_count % 50 == 0 {
                                println!(
                                    "[REINIT] Attempt #{}, backoff: {:?}",
                                    successful_reinitializations,
                                    recovery.current_backoff()
                                );
                            }
                        }
                ...
                Err(DomainError::ReInitializationRequired) => {
                    ...
                    println!(
                        "[ERROR] ReInitializationRequired, backing off: {:?}",
                        backoff
                    );
        ...
        println!(
            "  DeviceNotAvailable errors: {}",
            device_not_available_count
        );
        println!(
            "  ReInitializationRequired errors: {}",
            reinit_required_count
        );
        println!("  Effective FPS: {:.2}", effective_fps);
        println!();
        println!("Recovery Statistics:");
        println!(
            "  Total reinitializations: {}",
        ...
        if device_not_available_count > 0 {
            ...
            println!(
                "  ✓ Reinitialization executed: {} times",
                successful_reinitializations
            );
            ...
            if effective_fps >= min_expected_fps {
                println!(
                    "  ✓ Stable frame rate achieved: {:.2} FPS (target: {} FPS)",
                    effective_fps, min_expected_fps
                );
            } else {
                println!(
                    "  ⚠ Frame rate below target: {:.2} FPS (target: {} FPS)",
                    effective_fps, min_expected_fps
                );
                println!("    This may indicate frequent AccessLost errors");
                println!(
                    "    DeviceNotAvailable count: {}",
                    device_not_available_count
                );
            }
            ...
            println!(
                "  ✓ Stable frame rate: {:.2} FPS (device: {} Hz, threshold: {:.2})",
